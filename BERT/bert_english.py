...
Original file is located at
    https://colab.research.google.com/drive/1rtUJa-CtdMzwnqWkbMEAy5SZQAKER6BQ

## BERT English-Trained Dementia Classifier

Trains BERT (`bert-base-uncased`) on English conversational transcripts
and evaluates cross-lingual transfer to Tagalog.

### Design Choices
- **Pooling**: We use 'masked mean pooling' across all tokens in a transcript (instead of just the special `[CLS]` token). This method helps capture the full meaning of the transcript.
- **Learning rate**: The learning rate is chosen from a lower range (between 5e-6 and 3e-5), which is typically optimal for fine-tuning the model.
...
for seed in [s for s in SEEDS if s not in completed_seeds]:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(df_en["text"], df_en["label"])):
        ...
        both_f1_0.extend(f1_per[0]); both_f1_1.extend(f1_per[1]); both_rec_1.append(rec_per[1]); both_prec_1.append(seed_both_prec_1)
...
print("\n" + "=" * 75)
print("ENGLISH-TRAINED BERT — FINAL RESULTS (mean ± std across folds)")
print("=" * 75)
